...
cool: bin/main.o bin/tester.o
	$(LD) -framework AppKit -demangle -lto_library /Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/lib/libLTO.dylib -no_deduplicate -dynamic -arch x86_64 -macosx_version_min 10.14.0 -syslibroot /Library/Developer/CommandLineTools/SDKs/MacOSX.sdk -o cool $^ -L/usr/local/lib -lc++ -lSystem /Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/lib/clang/11.0.0/lib/darwin/libclang_rt.osx.a


print_predefines:
	clang -dM -E -x c++ - < /dev/null

bin/main.o: src/main.mm
	mkdir -p $(dir $@)
	clang $(C_FLAGS) src/main.mm -o bin/main.o

bin/tester.o: src/tester.m src/video.h
	mkdir -p $(dir $@)
	clang $(C_FLAGS) src/tester.m -o bin/tester.o
